...
updater = Updater(token='632514486:AAE5lWV0kYD1KrI1ONGfxGFU9ouLTHtgn-k')
...
def balance(bot,update):
	quote_page = requests.get('https://metropolitancoin.squarespace.com/')
...
def price(bot,update):
	...
	btc = soup[80:]
	sats = btc[:-2]
	bot.send_message(chat_id=update.message.chat_id, text="METRO is valued at {0} Δ {1} ≈ {2}".format(price,percent,sats) + " ฿")
...
def marketcap(bot,update):
	...
	name = name_box.text.replace("\n","")
	mc = re.sub(r'\n\s*\n', r'\n\n', name.strip(), flags=re.M)
	bot.send_message(chat_id=update.message.chat_id, text="The current market cap of METRO is valued at {0}".format(mc))
